...
- `src/pages/index.astro` — search-first front page (per docs/superpowers/specs/2026-06-12-homepage-search-hero-design.md): centered hero (no Arabic subline) with a GET form into dashboard ?q=, "/" focuses search, four topic chips deep-linking dashboard filters, then a full-width facts band (2x2 stat cards left, classification axes right, owner wants dense layouts, no big empty zones), jurisdiction tiles with SVG flags (src/components/Flag.astro), full-width month-grouped updates timeline with confidence badges (official=High green, secondary=Medium amber, pending=Pending). Contribute panel removed June 2026, owner manages entries himself; do not reintroduce public contribution CTAs. Motion: load choreography + IntersectionObserver scroll reveals (js-reveal class, once only) + hero glow, all behind prefers-reduced-motion; .home has overflow-x clip to contain the glow. Site-wide desktop cursor glow ring lives in Base.astro + global.css (native cursor never hidden). scripts/verify-home.mjs is a one-off Playwright check for this page. Scoped styles in-file.
